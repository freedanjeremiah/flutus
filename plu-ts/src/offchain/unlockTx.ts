...
import { htlcAddress, htlcScript, createRevealRedeemer } from "../../contracts/htlc";
...
export async function redeemHTLC(
  ...
  try {
    ...
    // Create the reveal redeemer
    const redeemer = createRevealRedeemer(secret);
    console.log("Created redeemer:", redeemer);
    ...
    console.log("Redemption transaction submitted successfully:", txHash);
    return txHash;
    
  } catch (error) {
    console.error("Detailed redemption error:", error);
    throw new Error(`Failed to redeem HTLC: ${error}`);
  }
}
